...
public class ButtonInteractionHandler extends ListenerAdapter {
    ...
    private void showTrophyDetail(ButtonInteractionEvent event, int trophyId) {
        try {
            ...
            event.replyEmbeds(trophy.createEmbed())
                .addActionRow(
    ...
    private void showTrophyWinners(ButtonInteractionEvent event, int trophyId) {
        try {
            ...
            event.replyEmbeds(eb.build())
                .addActionRow(
                    Button.primary("trophy_detail:" + trophyId, "Zurück zu Details")
                )
                .queue();
        } catch (SQLException e) {
            event.reply("Fehler beim Abrufen der Gewinner: " + e.getMessage())
                .setEphemeral(true)
                .queue();
        }
    }
    ...
    private void handleTrophyListPagination(ButtonInteractionEvent event, int page) {
        try {
            ...
            event.editMessageEmbeds(eb.build())
                .setActionRow(
                    Button.primary("trophy_list:" + (page - 1), "◀️ Zurück")
                        .withDisabled(page <= 1),
